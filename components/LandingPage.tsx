...
const FeaturedStationCard: React.FC<{station: typeof stations[0], index: number}> = ({station, index}) => (
    <div className="bg-gray-800/50 rounded-lg p-3 flex items-center gap-3 border border-gray-700/50 animate-fade-in" style={{animationDelay: `${1000 + index * 150}ms`, animationFillMode: 'backwards'}}>
        <img src={station.coverArt} alt={station.name} className="w-12 h-12 rounded-md object-cover flex-shrink-0" />
...
export const LandingPage: React.FC<LandingPageProps> = ({ onEnter }) => {
  return (
    <div className="min-h-screen bg-gray-950 text-white font-poppins">
      ...
      <section className="bg-gray-950 py-20 px-8 relative z-10">
          <div className="container mx-auto">
              <h2 className="text-3xl font-bold text-center mb-4 font-orbitron">Featured Stations</h2>
              <p className="text-center text-gray-400 mb-10 max-w-xl mx-auto">Here's a taste of what's playing right now. Dive in to discover thousands more.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                  {stations.slice(0, 4).map((station, index) => <FeaturedStationCard key={station.streamUrl} station={station} index={index} />)}
              </div>
          </div>
      </section>
